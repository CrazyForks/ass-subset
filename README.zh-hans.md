...
**ASS 子集化工具** 是由 蒙太奇字幕组 (MontageSubs) 开发的开源浏览器工具，用于在 ASS/SSA 字幕文件中管理字体嵌入与绘图指令。ASS/SSA 格式允许使用自定义字体来增强视觉效果，但跨平台安装字体不便。本工具在浏览器中直接实现字体嵌入，无需额外软件，跨平台可用；同时支持**字体子集化**，仅保留字幕实际使用的字符以进一步减小文件体积，并可将重复的绘图指令转换为字体以最大化压缩效果。

## 核心特色

- **浏览器中直接处理** — 所有文件在本地处理，支持离线使用，保护隐私，无需安装额外软件。  
- **预览与下载** — 处理后可即时预览优化结果，满意后下载优化后的字幕。  
- **符合无障碍标准** — 完整支持键盘导航与屏幕阅读器，满足 WCAG 及相关国际无障碍设计要求。  
- **原创设计的界面** — 现代化前端与直观交互，兼具独特美感与易用性。

## 功能

### 绘图指令子集化
将字幕中的 `\p1` … `\p0` 矢量绘图提取为内嵌字体；相同形状只存储一次并替换所有重复引用，显著减小文件体积并提升在低性能设备上的渲染兼容性；支持智能的增删操作。

### 第三方字体嵌入
扫描并嵌入字幕中使用的非系统字体，支持子集化（仅保留使用字符）或完整嵌入；完整保留字体名称表以确保播放器正确识别。

### 系统字体嵌入
对子集化后的系统字体进行嵌入，保证跨平台渲染一致性，无需用户手动安装字体。

### 移除嵌入字体
可删除字幕中已有的嵌入字体，同时保留已优化的绘图字体。

### 高级选项
- **保留 ASCII 字符** — 在子集中保留原字体的所有 ASCII 字符以提升播放器兼容性。  
- **完整字体嵌入** — 直接嵌入完整字体（无需重处理），但会增大文件体积。  
- **支持多种字体格式** — TTF、OTF、TTC、OTC、WOFF、WOFF2（暂不支持可变字体）
...
4. （可选）上传字体文件，或在 Chromium 浏览器中自动加载系统字体
